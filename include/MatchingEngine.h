...
private:
    ...
    // The per-order counters live in threadStats_ above and NOWHERE ELSE.
    //
    // There used to be three engine-wide atomics here — submittedTotal_,
    // processedTotal_, nextSubmitSequence_ — RMW'd on every single order. They
    // were padded apart behind a build flag on a false-sharing theory, which
    // could never have worked: the cost is not the neighbours sharing a line,
    // it is that ONE line is fetch_add'd by all N producers (submitted) and one
    // by all N workers (processed). At 8 threads that line ping-pongs 8 ways no
    // matter what surrounds it.
    //
    // They existed only for cold readers — getSubmittedCount/getProcessedCount,
    // waitForDrain, the checkpoint trigger — all of which now sum
    // threadStats_[] at read time. An O(numThreads) sum on a cold path is free;
    // two contended RMWs per order are not. Sequence ids come from a
    // thread-local block allocator in the .cpp (see nextSequenceId).
