> **Status update (2026-05-29):** Six software-only production-readiness improvements shipped. **395/395 tests passing** (was 392). New: (1) structured log call sites — 7 typed `obSink().log()` events wired in `OrderBook.cpp` (accepted, cancelled, rejected, trade fill, circuit breaker, trading state change); (2) `/readyz` k8s readiness probe endpoint (503 until warmup, then 200, auth-exempt); (3) exponential reconnect backoff in replication transport (500ms → 30s cap); (4) `RateLimiter::reconfigure()` for live SIGHUP rate updates with bucket reset; (5) journal auto-rotation via `OB_JOURNAL_MAX_SIZE_MB`; (6) three new test executables (AdminAuthTest, RateLimiterReconfigureTest, ReconnectBackoffTest). The old 2026-05-28 status remains below for reference.

> **Status update (2026-05-28):** Tier-1 exchange upgrade complete. All Phase 2–6 features now implemented and tested: **392/392 tests passing** (was 181). New since 2026-05-19: MOC/LOC on-close orders (`onCloseOrders_` queue, `cancelLocOrders()` after uncross, `AuctionClose` release), LMM/DMM venue-specific allocation (`ParticipantRole` enum, 40% ProRata floor guarantee, rounding-remainder priority), SIGHUP hot-reload (`--config PATH` flag, `g_reload_config` atomic, main-loop reload), and full microstructure research infrastructure (SimulationDriver, VPIN, Huang-Stoll/Glosten-Harris spread decomposition, Almgren-Chriss execution model, PaperTrader). Previously completed since the prior update: Auction state machines (PreOpen/AuctionOpen/AuctionClose/VolatilityAuction), Advanced order types (MIT/OCO/MOC/LOC), Hierarchical risk (HierarchicalRiskManager — Firm/Account/Strategy/Trader tiers + fat-finger), CAT audit trail (CatReporter), FeeEngine, LatencyHistogram, ProtocolVersion. TLA+ spec count: 7 → **12** (added Auction, EpochDurability, FixSession, Oco, Risk — each with broken-variant sanity check). What remains: the perf-cliff work (Phase 1 Weeks 1–4), Expanded SMP variants (Cancel-Maker/Decrement-and-Cancel), 24h+ TSan soak (harness exists, clock-time bound), x86 bare-metal benchmarks (need hardware), cross-DC replication, and the clearing/regulatory-submission items (need organizational infrastructure).
